...
export default function MiniNav({ onNavigate, triggerHaptic }) {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-30 bg-slate-900/95 dark:bg-slate-950/95 backdrop-blur-xl border-t border-slate-700/50 pb-[env(safe-area-inset-bottom)]">
            <div className="flex items-center justify-around max-w-md sm:max-w-lg md:max-w-xl mx-auto px-2 py-1.5">
                {NAV_ITEMS.map(nav => (
                    <button
                        key={nav.id}
                    ...
                    >
                        <nav.icon size={18} />
                        <span className="text-[9px] sm:text-xs font-bold">{nav.label}</span>
                    </button>
                ))}
            </div>
        </div>
